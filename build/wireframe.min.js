...
b:t.b};this.hsl={h:c.h,s:c.s,l:c.l};this.alpha=m}function k(b,m,a){b<m?b=m:b>a&&(b=a);return b}var e,a,h,l;c.prototype.lighten=function(b){var m=this.hsl;b=m.l+b;100<b&&(b=100);return new c({h:m.h,s:m.s,l:b},this.alpha)};c.prototype.darken=function(b){var m=this.hsl;b=m.l-b;0>b&&(b=0);return new c({h:m.h,s:m.s,l:b},this.alpha)};c.prototype.toString=function(){var b=this.rgb.r.toString(16),m=this.rgb.g.toString(16),a=this.rgb.b.toString(16);1===b.length&&(b="0"+b);1===m.length&&(m="0"+m);1===a.length&&
(a="0"+a);return"#"+b+m+a};e=function(b,m,a){function c(b,m,a){0>a&&(a+=1);1<a&&(a-=1);return a<1/6?b+(m-b)*a*6:.5>a?m:a<2/3?b+(m-b)*(2/3-a)*6:b}var h=a/100,e=m/100;if(0===m)return b=255*h,{r:b,g:b,b:b};m=50>=a?h*(1+e):h+e-h*e;h=2*h-m;b/=360;return{r:Math.round(255*c(h,m,b+1/3)),g:Math.round(255*c(h,m,b)),b:Math.round(255*c(h,m,b-1/3))}};a=function(b,m,a){b/=255;m/=255;a/=255;var c=Math.max(b,m,a),h=Math.min(b,m,a),e=Math.round((h+c)/2*100);100<e&&(e=100);0>e&&(e=0);var l;if(h===c)return{h:0,s:0,
l:e};l=50>=e?(c-h)/(c+h):(c-h)/(2-c-h);var d=(c-b)/(c-h),g=(c-m)/(c-h);a=(c-a)/(c-h);b=(b===c?a-g:m===c?2+d-a:4+g-d)/6%1;0>b&&(b+=1);b=Math.round(360*b);l=Math.round(100*l);360<b&&(b=360);0>b&&(b=0);100<l&&(l=100);0>l&&(l=0);return{h:b,s:l,l:e}};h=function(b){var m,a,c,h=1,l,d=!1;l=b.substr(0,3);if("hsl"===l)(l=/hsla?\(\s*(-?\d+)\s*,\s*(-?\d+)%\s*,\s*(-?\d+)%\s*(,\s*(-?\d+(\.\d+)?)\s*)?\)/g.exec(b))?(m=parseInt(l[1],10),a=parseInt(l[2],10),c=parseInt(l[3],10),"a"===b[3]&&(h=parseFloat(l[5])),m=Math.abs(m%
360),a=k(a,0,100),c=k(c,0,100),l=e(m,a,c),m=l.r,a=l.g,c=l.b):d=!0;else if("rgb"===l)if(l=/rgba?\((-?\d+%?)\s*,\s*(-?\d+%?)\s*,\s*(-?\d+%?)(,\s*(-?\d+(\.\d+)?)\s*)?\)/g.exec(b)){var g=l[1],f=l[2],n=l[3];m=parseInt(l[1],10);a=parseInt(l[2],10);c=parseInt(l[3],10);if("%"===g[g.length-1]||"%"===f[f.length-1]||"%"===n[n.length-1])"%"===g[g.length-1]&&"%"===f[f.length-1]&&"%"===n[n.length-1]?(m=Math.floor(m/100*255),a=Math.floor(a/100*255),c=Math.floor(c/100*255)):d=!0;m=k(m,0,255);a=k(a,0,255);c=k(c,0,
255);"a"===b[3]&&(h=parseFloat(l[5]))}else d=!0;else"#"===b[0]?(l=b.substr(1),3===l.length?(m=parseInt(l[0]+l[0],16),a=parseInt(l[1]+l[1],16),c=parseInt(l[2]+l[2],16)):6===l.length?(m=parseInt(l[0]+l[1],16),a=parseInt(l[2]+l[3],16),c=parseInt(l[4]+l[5],16)):d=!0):d=!0;h=k(h,0,1);if(d)throw"ColourError: Something went wrong. Perhaps "+b+" is not a legal CSS color value";return{r:m,g:a,b:c,a:h}};l={black:{r:0,g:0,b:0,h:0,s:0,l:0},silver:{r:192,g:192,b:192,h:0,s:0,l:75},gray:{r:128,g:128,b:128,h:0,s:0,
...
f*e;this._zaxis.y=-c;this._zaxis.z=e*a;n.fromArrayLG([this._xaxis.x,this._yaxis.x,this._zaxis.x,0,this._xaxis.y,this._yaxis.y,this._zaxis.y,0,this._xaxis.z,this._yaxis.z,this._zaxis.z,0,-this._xaxis.dot(d),-this._yaxis.dot(d),-this._zaxis.dot(d),1],this.view_matrix)};d.prototype.look=function(d,c){this.rotation.yaw-=d;0>this.rotation.yaw?this.rotation.yaw+=p:this.rotation.yaw>p&&(this.rotation.yaw-=p);this.rotation.pitch-=c;0>this.rotation.pitch?this.rotation.pitch+=p:this.rotation.pitch>p&&(this.rotation.pitch-=
...
this._back_buffer_image.data;var b=4*c;a[b]=f.r;a[b+1]=f.g;a[b+2]=f.b;a[b+3]=255;this._depth_buffer[c]=d}};d.prototype.drawEdge=function(c,a,d){var f=Math.abs;if(c.x>=a.x){var b=c;c=a;a=b}var b=c.x,m=c.y,g=c.z,f=Math.max(f(a.x-c.x),f(a.y-c.y),f(a.z-c.z)),k=(a.x-c.x)/f,n=(a.y-c.y)/f;c=(a.z-c.z)/f;for(a=0;a<f;a++)this.drawPixel(Math.floor(b),Math.floor(m),g,d),b+=k,m+=n,g+=c};d.prototype.quickLine=function(c,a){var d=Math.round(c.x+this._x_offset),f=Math.round(c.y+this._y_offset),b=Math.round(a.x+this._x_offset),
m=Math.round(a.y+this._y_offset);this.ctx.moveTo(d,f);this.ctx.lineTo(b,m)};d.prototype.drawTriangle=function(c,a,d,f){this._quickdraw?(this.quickLine(c,a),this.quickLine(a,d),this.quickLine(d,c)):(this.drawEdge(c,a,f),this.drawEdge(a,d,f),this.drawEdge(d,c,f))};d.prototype.fillTriangle=function(c,a,d,f){var b=c.x,m=a.x,g=d.x,k=c.y,n=a.y,p=d.y;c=c.z;a=a.z;d=d.z;var q=0-this._x_offset,r=this.width-this._x_offset,s=0-this._y_offset,u=this.height-this._y_offset,v=Math.floor(Math.min(b,m,g));v<q&&(v=
...
this._wv2,this._wv3,n.rgb):1===this._draw_mode&&(d.subtractLG(this._v1t,this._light_direction),this._light_direction.normalizeLG(this._light_direction),p=this._norm.dot(this._light_direction),n=n.lighten(15*p),this.fillTriangle(this._wv1,this._wv2,this._wv3,n.rgb)))}}this._quickdraw&&0===this._draw_mode?(this.ctx.stroke(),this.ctx.closePath()):(this._back_buffer_ctx.putImageData(this._back_buffer_image,0,0),this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height),this.ctx.drawImage(this._back_buffer,
